...
xargs -d "\n" grep -li 'lzw_decompress(' < /tmp/av-php.tmp
echo "Searching in php files [7]..."
xargs -d "\n" grep -lie 'TrustlinkClient\|SAPE_base\|SAPE_client' < /tmp/av-php.tmp
echo "Searching in php files [8]..."
xargs -d "\n" grep -li 'MailTo = base64_decode' < /tmp/av-php.tmp
echo "Searching in php files [9]..."
xargs -d "\n" grep -li 'find suid files in current dir' < /tmp/av-php.tmp
echo "Searching in php files [10]..."
xargs -d "\n" grep -lie "base64_decode([\"']" < /tmp/av-php.tmp
rm /tmp/av-php.tmp
